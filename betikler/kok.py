...
def kaynakKonumunuYapılandır():
    ...
    if path.exists(uretimKonumu) != True:
        os.mkdir(uretimKonumu)
    os.chdir(uretimKonumu)
...
def valgrind(yazilim, seçimler):
    ...
    Renk.Sari("%s:%s valgrind hata ayıklaması sonlandı." % (yazilimAdi, yazilimSürümü))


def valgrindAyrıntılı(yazilim, seçimler):
    Renk.Sari("%s:%s valgrind ayrıntılı hata ayıklaması:" % (yazilimAdi, yazilimSürümü))
    argumanlar = (
