...
struct _orst_birim
{
  ...
  {
    int           belge;
    orst_harfler* Degerler;
    orst_harfler* Bilgi;
    orst_harfler* Turler;
    orst_harfler* Genel;
    orst_harfler* Ayiklama;
  } cikti;
  orst_imge_yigin_dizisi        siralama;
  orst_altyapi_islemleri        altyapi;
  d32                           no;
  orst_kume_imge*               Degerler;
  orst_kume_tur*                Turler;
...
void        orsi_birim_turAtfiEkle(orst_birim*, orst_imge*);
void orsi_birim_Yazdir(struct _orst_uretim*, orst_birim*, struct _orst_urun*);
void
orsi_birim_AraYapilandir(struct _orst_uretim*, orst_birim*, struct _orst_urun*);
