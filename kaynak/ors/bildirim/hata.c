...
{
  int d    = 0;
  sey Imge = Bildiri->icerik.Bildiri->Konum;
  ...
  if(Imge)
  {
    ...
    orsi_konum_Bilgi(&Imge->kesit.Bas->konum, &_konum[d], (ORS_BELLEK_4096));
    ...
    fprintf(stderr,
            ...
            _bellek);
    _bellek[0] = 0;
    d          = orsi_bildiri_Hata(Bildiri->icerik.Bildiri->_kod[0],
                          &_bellek[0],
                          ORS_BELLEK_4096);
    ...
    fprintf(stderr, "%s\n", Bildiri->nesne.icerik.Metin->Nesneler);
    fflush(NULL);
  }
}
  ...
  orst_derleme* Derleme, int kod, orst_imge* Konum, const char* _bicim, ...)
{
  ...
  orsh_imge_yeni_nolu(
    Hata,
    Derleme,
    (Konum ? Konum->kesit.Bas : orsh_imge_sanal(Derleme)->kesit.Bas),
    Ors_Imge_Bildiri);
  orsh_nesne_yapilandir(Derleme,
                        Hata,
                        ORS_BELLEK_4096,
                        Ors_Nesne_Anlam_Bildirim);
  mimari boyut   = ORS_BELLEK_4096;
  char*  _bellek = Hata->nesne.icerik.Metin->Nesneler;

  va_list Argumanlar;
  va_start(Argumanlar, _bicim);
  Hata->nesne.icerik.Metin->boyut
    = vsnprintf(_bellek, boyut, _bicim, Argumanlar);
  va_end(Argumanlar);

  orsh_temiz_altuye(Hata->icerik.Bildiri);
  Hata->icerik.Bildiri->Konum   = Konum;
  Hata->icerik.Bildiri->_kod[0] = kod;

  orsh_sabit_dizi_ekle(Derleme->bildiriler.hatalar, Hata);
  orsh_derleme_durdur(Derleme, kod);
  orsh_cozumleme_durdur(Derleme, kod);
  Derleme->uretim.t.t = kod;
  //  orsi_bildiri_HataYazdir(Derleme, Hata);
  return Hata;
}
...
orsi_bildiri_Hata(int d, char _arabellek[], size_t uzunluk)
{
  ...
  switch(d)
  {
    ...
    case Ors_Hata_Is_Dosya_Bilgisi:
      boyut
        += snprintf(_arabellek, uzunluk - 1, "işletim_sistemi:dosya_bilgisi");
      break;
    case Ors_Hata_Is_Hafiza_Izdusum:
      boyut
        += snprintf(_arabellek, uzunluk - 1, "işletim_sistemi:hafıza_izdüşüm");
      break;
    ...
    case Ors_Hata_Tarama_Tanimsiz_Simge:
      ...
      break;
    case Ors_Hata_Tarama_Akis_Simgesi:
      boyut += snprintf(_arabellek, uzunluk - 1, "tarama:akış");
      break;
    ...
    case Ors_Hata_Cozumleme:
      boyut += snprintf(_arabellek, uzunluk - 1, "çözümleme:çözümleme");
      break;
    ...
    case Ors_Hata_Cozumleme_Beklenmeyen_Simge:
      boyut += snprintf(_arabellek, uzunluk - 1, "çözümleme:beklenmeyen_simge");
      break;
    ...
    case Ors_Hata_Uretim_KutuphaneBulunamadi:
      boyut += snprintf(_arabellek, uzunluk - 1, "üretim:kütüphane_bulunamadı");
      break;
  ...
  boyut
    += snprintf(&_arabellek[boyut], uzunluk - ((size_t)boyut) - 1, ":'%u'", d);
  return boyut;
}
