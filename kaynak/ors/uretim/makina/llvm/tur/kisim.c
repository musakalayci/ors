...
{
  sey Atif = TurKismi->Gosterge->nesne.bulunan.Turu;
  if(!Atif)
    return BOS;
...
orsi_uretim_TurKismi(orst_uretim* Uretim, orst_imge_turKismi* TurKismi)
{
  ...
  TurKismi->Oz->nesne.bulunan.Oz   = TurKismi->Oz;
  ...
  if(TurKismi->Dizi)
  {
    ...
    derece                             = 0;
    orsh_imge_nesne_dizi(TurKismi->Oz) = TurKismi->Dizi->boyut;
    orst_imge* Seviye                  = TurKismi->Dizi->Nesneler[0];
    ...
    for(int i = 1; i < TurKismi->Dizi->boyut; i++)
    {
      ...
      Seviye->nesne.bulunan.Turu        = TurKismi;
      orsh_nesne_derece(&Seviye->nesne) = i + 1;
      orsh_imge_nesne_dizi(Seviye)      = i;
