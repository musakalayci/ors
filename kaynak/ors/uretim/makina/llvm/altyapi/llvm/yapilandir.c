...
{
  sey Altyapi
    = orsh_altyapi_islemi("memset", Ors_Altyapi_I_Hafiza_Memset, "p0i8.i64");
  ...
  orsh_dizi_ekle(Altyapi->turler, Ikinci);
  orsh_dizi_ekle(Altyapi->turler, Ucuncu);
  orsh_dizi_ekle(Altyapi->turler, Dorduncu);
}

void
orsi_uretim_altyapiYapilandir(orst_uretim* Uretim, orst_birim* Birim, char* _ad)
{
  ...
  sey Altyapi           = orsh_altyapi_islemi("memcpy",
